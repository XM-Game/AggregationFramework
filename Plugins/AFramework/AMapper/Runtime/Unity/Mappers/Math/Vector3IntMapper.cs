// ==========================================================
// 文件名：Vector3IntMapper.cs
// 命名空间: AFramework.AMapper.Unity
// 依赖: UnityEngine, AFramework.AMapper
// 功能: Vector3Int 类型映射器，支持 Vector3Int 与其他向量类型的转换
// ==========================================================
...
namespace AFramework.AMapper.Unity
{
    /// <summary>
    /// Vector3Int 类型映射器
    /// <para>支持 Vector3Int 与 Vector3、Vector2Int、int[] 等类型的双向转换</para>
    /// <para>Vector3Int type mapper supporting bidirectional conversion with Vector3, Vector2Int, int[], etc.</para>
    /// </summary>
    /// <remarks>
    /// 设计原则：
    /// <list type="bullet">
    /// <item>单一职责：仅负责 Vector3Int 相关类型的映射</item>
    ...
    /// 支持的映射类型：
    /// <list type="bullet">
    /// <item>Vector3Int ↔ Vector3（整数与浮点数转换）</item>
    /// <item>Vector3Int ↔ Vector2Int（丢弃或补充 z 分量）</item>
    /// <item>Vector3Int ↔ int[]（数组长度至少为 3）</item>
    /// </list>
    /// </remarks>
    public sealed class Vector3IntMapper : IObjectMapper
    {
        ...
        {
            if (source == null)
                return GetDefaultValue(destination?.GetType() ?? typeof(Vector3Int));

            try
            {
                var sourceType = source.GetType();
                var destType = destination?.GetType() ?? typeof(Vector3Int);
                ...
                if (sourceType == typeof(Vector3Int))
                {
                    ...
                    if (destType == typeof(int[]))
                        return new int[] { vector3Int.x, vector3Int.y, vector3Int.z };
                }
                ...
                if (destType == typeof(Vector3Int))
                {
                    ...
                    if (sourceType == typeof(int[]))
                    {
                        ...
                        if (array.Length < 3)
                            throw new MappingException($"数组长度不足，至少需要 3 个元素才能转换为 Vector3Int / Array length insufficient, at least 3 elements required to convert to Vector3Int");
            ...
            {
                throw new MappingException($"Vector3Int 映射失败 / Vector3Int mapping failed: {ex.Message}", ex);
            }
        }
