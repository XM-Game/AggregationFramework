// ==========================================================
// 文件名：QuaternionMapper.cs
// 命名空间: AFramework.AMapper.Unity
// 依赖: UnityEngine, AFramework.AMapper
// 功能: Quaternion 类型映射器，支持 Quaternion 与其他类型的转换
// ==========================================================
...
namespace AFramework.AMapper.Unity
{
    /// <summary>
    /// Quaternion 类型映射器
    /// <para>支持 Quaternion 与 Vector4、Vector3（欧拉角）、float[] 等类型的双向转换</para>
    /// <para>Quaternion type mapper supporting bidirectional conversion with Vector4, Vector3 (Euler angles), float[], etc.</para>
    /// </summary>
    /// <remarks>
    /// 设计原则：
    /// <list type="bullet">
    /// <item>单一职责：仅负责 Quaternion 相关类型的映射</item>
    ...
    /// 支持的映射类型：
    /// <list type="bullet">
    /// <item>Quaternion ↔ Vector4（xyzw 分量对应）</item>
    /// <item>Quaternion ↔ Vector3（欧拉角转换）</item>
    /// <item>Quaternion ↔ float[]（数组长度至少为 4）</item>
    /// </list>
    /// 
    /// 注意事项：
    /// <list type="bullet">
    /// <item>Vector3 与 Quaternion 转换时，Vector3 表示欧拉角（度数）</item>
    /// <item>欧拉角转换可能存在万向锁问题</item>
    /// </list>
    /// </remarks>
    public sealed class QuaternionMapper : IObjectMapper
    {
        ...
        public bool IsMatch(TypePair typePair)
        {
            ...
            {
                return destType == typeof(Vector4) ||
            ...
            {
                return sourceType == typeof(Vector4) ||
        ...
        {
            if (source == null)
                return GetDefaultValue(destination?.GetType() ?? typeof(Quaternion));

            try
            {
                var sourceType = source.GetType();
                var destType = destination?.GetType() ?? typeof(Quaternion);
                ...
                if (sourceType == typeof(Quaternion))
                {
                    ...
                    if (destType == typeof(Vector3))
                        return quaternion.eulerAngles; // 转换为欧拉角
                ...
                if (destType == typeof(Quaternion))
                {
                    ...
                    if (sourceType == typeof(Vector3))
                    {
                        ...
                        return Quaternion.Euler(vector3); // 从欧拉角创建
                    }
                    ...
                    if (sourceType == typeof(float[]))
                    {
                        ...
                        if (array.Length < 4)
                            throw new MappingException($"数组长度不足，至少需要 4 个元素才能转换为 Quaternion / Array length insufficient, at least 4 elements required to convert to Quaternion");
            ...
            {
                throw new MappingException($"Quaternion 映射失败 / Quaternion mapping failed: {ex.Message}", ex);
            }
        }
        ...
        private object GetDefaultValue(Type type)
        {
            ...
            if (type == typeof(Vector4))
                return new Vector4(0, 0, 0, 1); // Quaternion.identity 的 xyzw
